...
import {connections, db, type Session, sessions, users, verifications} from "#app/db";
...
test('when a user is logged in, it creates the connection', async () => {
	...
	if (!session) {
		return expect(session).toBeTruthy()
	}
...
test('when a user exists with the same email, create connection and make session', async () => {
	...
	const session = await setupUser({ ...createUser(), email })
	...
	await expect(response).toHaveSessionForUser(session.userId)
})
